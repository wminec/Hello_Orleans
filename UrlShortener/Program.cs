...
app.MapGet("/go/{shortendRouteSegment:required}",
static async (IGrainFactory grains, string shortendRouteSegment) =>
{
    // Retrieve the grain using the shortened ID and url to the original URL
    var shortenerGrain =
        grains.GetGrain<IUrlShortenerGrain>(shortendRouteSegment);
